...
pub fn ensure_pj_thread_registered() {
    ...
    REGISTERED.with(|reg| {
        if !reg.get() {
            ...
            unsafe {
                pj_thread_register(thread_name.as_ptr(), thread_desc.as_mut_ptr(), &mut thread);
            }
            reg.set(true);
        }
    });
...
pub fn init(loglevel : u32 , incomming_call_behaviour: OnIncommingCall ) -> Result<i8,TelephonyError> { 
    ...
    let status : pj_status_t;    
    let mut cfg =  unsafe {
        let mut cfg: MaybeUninit<pjsua_config> = MaybeUninit::uninit();
	...
	let mut log_cfg =  unsafe {
        let mut log_cfg: MaybeUninit<pjsua_logging_config> = MaybeUninit::uninit();
	...
	status = unsafe{pjsua_init(&cfg, &log_cfg, std::ptr::null())};
    if (status != pj_constants__PJ_SUCCESS as i32){ 
...
pub fn add_transport(port: u32, mode: TransportMode ) -> Result <i8,TelephonyError>{
    ...
    let mut transport_cfg =  unsafe {
        let mut transport_cfg: MaybeUninit<pjsua_transport_config> = MaybeUninit::uninit();
        pjsua_transport_config_default(transport_cfg.as_mut_ptr());
        transport_cfg.assume_init()
    };    

    transport_cfg.port = port;
    let transportMode = match(mode){
        TransportMode::TCP  => pjsip_transport_type_e_PJSIP_TRANSPORT_TCP,
        TransportMode::TLS  => pjsip_transport_type_e_PJSIP_TRANSPORT_TLS,
        TransportMode::UDP6 => pjsip_transport_type_e_PJSIP_TRANSPORT_UDP6,
        TransportMode::UDP  => pjsip_transport_type_e_PJSIP_TRANSPORT_UDP,
        TransportMode::TCP6 => pjsip_transport_type_e_PJSIP_TRANSPORT_TCP6,
        TransportMode::TLS6 => pjsip_transport_type_e_PJSIP_TRANSPORT_TLS6
    };

    let mut status =  unsafe {
        let mut transport_id: MaybeUninit<pjsua_transport_id> = MaybeUninit::uninit();
        pjsua_transport_create( transportMode, &transport_cfg, transport_id.as_mut_ptr())
    };
...
pub fn accountSetup(username : String, uri : String, password : String) -> Result<i8,TelephonyError> {
    ...
    let mut acc_cfg =  unsafe {
        let mut acc_cfg: Box<MaybeUninit<pjsua_acc_config>> = Box::new(MaybeUninit::uninit());
    ...
    acc_cfg_ref.cred_info[0].data_type = pjsip_cred_data_type_PJSIP_CRED_DATA_PLAIN_PASSWD.try_into().unwrap();
    acc_cfg_ref.cred_info[0].data = data_pj_str_t;

    let acc_id : pjsua_acc_id;
    acc_id = 0 ;
    status = unsafe {pjsua_acc_add(acc_cfg_ref, pj_constants__PJ_TRUE.try_into().unwrap(), acc_id as *mut i32 )} ;
...
pub fn make_pj_str_t(input : String ) -> Result<pj_str_t,TelephonyError> {
...
// fn aux_on_incomming_call() -> extern "C" fn(acc_id: pjsua_acc_id, call_id: pjsua_call_id, rdata: *mut pjsip_rx_data) {
//     on_incoming_call
// }

extern "C" fn on_incoming_call(acc_id: pjsua_acc_id, call_id: pjsua_call_id, rdata: *mut pjsip_rx_data) {
    // let ci = unsafe {
    //     let mut ci : MaybeUninit<pjsua_call_info> = MaybeUninit::uninit();
    //     pjsua_call_get_info(call_id, ci.as_mut_ptr());
    //     ci.assume_init()
    // };
    unsafe{ pjsua_call_answer(call_id, 200, std::ptr::null(), std::ptr::null()); } 
    println!("The call id is: {}", call_id);
}

extern "C" fn on_incoming_call_ignore(acc_id: pjsua_acc_id, call_id: pjsua_call_id, rdata: *mut pjsip_rx_data) {
    // let ci = unsafe {
    //     let mut ci : MaybeUninit<pjsua_call_info> = MaybeUninit::uninit();
    //     pjsua_call_get_info(call_id, ci.as_mut_ptr());
    //     ci.assume_init()
...
extern "C" fn on_call_state(call_id: pjsua_call_id, e: *mut pjsip_event){
    let ci = unsafe {
        let mut ci : MaybeUninit<pjsua_call_info> = MaybeUninit::uninit();
        pjsua_call_get_info(call_id, ci.as_mut_ptr());
        ci.assume_init()
...
pub fn make_call(phone_number: &str, domain : &str ) -> Result<i8,TelephonyError>{
    ...
    let call_extension_pj_str_t = pj_str_t { slen: len , ptr: call_extension_myptr };
    ...
    let opt = 0 as *mut pjsua_call_setting;
    let make_call_restult = unsafe {pjsua_call_make_call( 0 , ptr_call_extension_pj_str_t , opt, user_data_null, 0 as *mut  pjsua_msg_data , 0 as *mut pjsua_call_id)};
...
pub fn send_dtmf(digit:u32) -> Result<i8,TelephonyError> {
    ...
    let dtmf_tones = pjsua_call_send_dtmf_param {
        method   : pjsua_dtmf_method_PJSUA_DTMF_METHOD_RFC2833,
        duration : PJSUA_CALL_SEND_DTMF_DURATION_DEFAULT,
        digits   : digits_pj_str_t
    };
    let status = unsafe{ pjsua_call_send_dtmf(0, &dtmf_tones )};
    if status!=0{
...
fn error_exit(err_msg: &str ){
    ...
    unsafe{pjsua_perror(thisfile, err  , 2)};
...
pub fn destroy_telephony()-> Result<i8,TelephonyError>{
    ...
    let status = unsafe{pjsua_destroy()};
    if status!=0{
